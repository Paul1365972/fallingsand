...
- **Input**: `game/input.rs` owns every raw key/mouse read — one hardcoded keymap, one input context (chat > paused > inventory > gameplay), flushed as one `InputFrame` per fixed tick.
