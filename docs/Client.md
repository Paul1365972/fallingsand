...
- **Settings page**: reachable from the main menu and the pause menu (its own opaque input context, `Escape` closes back to whatever is beneath). Toggles fullscreen, vsync, render mode (pixel-perfect/smooth/retro camera snapping), and UI scale (75–150%, via bevy's `UiScale`); all persist to `saves/settings.txt` as `key=value` lines and reapply on boot through the `ApplyWindow` effect.
