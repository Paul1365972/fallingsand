...
A cell is a compact heap-free value: material, velocity, shade, and a runtime flags byte — the tick-local moved stamp, stress tag, and raster ownership marker, never persisted. Every cell is a particle — velocity drives all energetic movement. Burning is a material, not a flag: a lit fuel transmutes into its synthesized burning twin and probabilistic burnout *is* the burn duration; there is no per-cell HP.
...
A moving cell integrates cardinal components along its velocity vector; fractional speed uses a tick-seeded local chance and reach stays inside the 64-cell window. Movement starts with a fixed cardinal-step budget; a momentum-changing swap recomputes its remaining direction and can shorten, but never extend, that budget. A swap marks both cells moved: moved matter cannot be displaced again that tick, moved air still admits traversal, and a mover refused by moved matter holds velocity and retries. Powders and gases use density admission and contact impulses. Whenever a moving cell displaces liquid, both cells keep their density-weighted center velocity. The square root of normalized density contrast carries retained relative speed forward; its complementary square root turns the remainder into a tick-hashed left-or-right wake, keeping the retained energy magnitude unchanged. A downward liquid-liquid exchange also gains an opposing transverse component from the density potential that submerged liquids do not receive as gravity; powder already carries its buoyant gravitational velocity into the same interaction. A second liquid merely contributes its own impact limit. Incoming water therefore penetrates and meanders through oil across both energetic and passive exchanges, while sediment leaves localized wakes along each grain path. Quantized momentum is never amplified—the unrepresentable remainder may dissipate—and added liquid wake energy is bounded by the gravitational potential released by the exchange.

A resting liquid first takes a strictly energy-lowering down or diagonal swap; a diagonal descent carries a conservatively bounded horizontal launch. Otherwise an exposed interface particle may take one zero-velocity neutral horizontal swap onto a supported lower column. Its direction alternates by material and row: successive staircase levels cannot all park uphill, while the indivisible remainder of a balanced surface compacts into a fixed two-row band. The optional passive-flow rate gates only these resting proposals. An exchange between two liquids uses the slower participant's rate multiplied by the square root of `|ρa−ρb|/max(ρa,ρb)`: density selects the downhill direction and softly scales its drive without making close pairs inert. Adjacent columns settle within one cell, while a balanced, fully stratified body is a fixed point. A settled capped gas mirrors the fall upward and seeps along ceilings into open pockets; steam condenses back to water so gas pockets resolve.
...
| Flags byte | runtime per-cell flags — the tick-local moved stamp, stress tag and cell-ownership marker; never persisted |
