...
- **Ignite**: a `hot` cell transmutes adjacent flammables into their embers at their `ignite` rate, keeping velocity and shade (igniting oil keeps flowing). Without adjacent oxygen both ignition and burn scale by the fuel's `sealed_burn` fraction, so deep burn is surface burn in slow motion; `0` is surface-only (oil) — never catches while sealed and snuffs out the instant it loses air. Heat splits by lifetime: **transient** (`fire`, `burning_*`) ignites in the interaction sim; **persistent** (`lava`) never burns out, so it ignites from the random-tick sim — slower but sleep-independent. No cell uses both.
