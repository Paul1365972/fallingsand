...
- **Weight** — powders take gravity minus buoyancy from the medium they displace and gases rise. A liquid exposed to a lower-density cell below takes gravity once per tick, whether falling, traveling sideways, or rising; upward motion therefore slows naturally. Density inversion uses the same local density ordering.
...
- **Liquid potential** — a resting liquid only falls into a less-dense target; horizontal interface swaps are neutral. Existing velocity may carry a particle upward or through a dynamic interface, while gravity, drag, and impact determine how long that motion survives. Swaps never add velocity.
...
A moving cell integrates cardinal components along its velocity vector; fractional speed uses a tick-seeded local chance and reach stays inside the 64-cell window. A swap marks both cells moved: moved matter cannot be displaced again that tick, moved air still admits traversal, and a mover refused by moved matter holds velocity and retries. Powders and gases use density admission and contact impulses. Liquid admits empty cells, gases, and other liquids, never powder: incoming water therefore collides with sediment. Denser powder may displace liquid downward; both particles take their density-weighted common velocity before swapping, conserving momentum while dissipating energy. The displaced liquid then resolves its downward collision sideways, producing a local sediment wake.

A resting liquid first takes a strictly energy-lowering down or diagonal swap. Otherwise an exposed interface particle may take one neutral horizontal swap onto a supported lower column or through a lighter liquid layer; each interface row has a stable direction so defects travel instead of random-walking. The optional passive-flow rate gates only these resting proposals. Adjacent columns settle within one cell, while a perfectly balanced body is a fixed point. A settled capped gas mirrors the fall upward and seeps along ceilings into open pockets; steam condenses back to water so gas pockets resolve.
