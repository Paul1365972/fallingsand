...
- **particles** — `ParticleSpawn` events (position, velocity, RGB) the server emits for dig spray. The server owns when and where they spawn; the frame carries only per-tick spawn events within interest, never simulated particle state. Emission runs on second-based accumulators (tick-rate independent) and is capped per tick, so a quiet world sends none. The client integrates and renders them with fixed gravity and lifetime.

Client -> server normally sends one `InputFrame` per client fixed tick: the held `InputState` plus ordered one-shot `InputAction`s. Held state is a plain latest snapshot of what is currently held; coalescing frames replace it wholesale, so the newest sample always wins and a press+release inside one window resolves to release. Edge-sensitive intents ride `InputAction`s, never the held snapshot. A client transition that cancels held control (menu opened, avatar incapacitated) neutralizes the snapshot and emits one immediate neutral frame, stopping movement at once rather than waiting on the 0.5 s decay. Session count, handshake lifetime, frame size, messages drained per tick, and actions per frame are bounded. The client carries excess actions into later frames; the server rejects an over-limit frame rather than silently dropping actions. Held input decays to neutral after 0.5 s without frames. Accepted intents live in the current player's `PlayerControl`; the authoritative session binding rejects input from a superseded connection, and takeover or lifecycle transitions clear already queued work. A new discrete input is a new `InputAction` variant, never a new message.
...
The client resolves one identity once at startup and holds it for the session: a durable secret and a mutable display name, each field resolved by the same precedence of CLI/config override, then stored value, then generation (a fresh key, or a name derived from the uuid). Name edits mutate that single resolved identity and persist the name alone, never minting a secret. An externally supplied key is never written back to storage.
