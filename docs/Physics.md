...
- **Bodies are terrain**: the CA runs reactions, decay, and combustion on them (a fallen tree burns). Any write unflagging a body cell — ignition to an ember included — feeds a damage queue; the bodies pass reconciles it before stepping — solid products re-adopted into the body's buffer (a moving log keeps its fire), bodies split by connectivity or despawn when empty.
