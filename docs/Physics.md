...
- **Motion is swept and exact** — one combined translation-and-rotation traversal over 64 authoritative lattice orientations; continuous turn is only the accumulator between them. A step crosses at most one cell per axis, one cell of rim travel, and one orientation, so no intermediate raster goes untested. The reversible lattice rotation keeps every slot unique.
