...
Movement tuning per phase: powders author drag, friction, topple (deterministic static-start susceptibility and per-tick kinetic keep), and deflect; liquids author drag, impact retention, and an optional passive-flow rate. Impact retention controls blocked redirection and how much relative speed remains when any denser cell displaces the liquid; a second liquid contributes the lower retention. Retained interface speed splits between forward penetration and transverse wake from normalized density contrast: the forward factor is its square root and the transverse factor is the complementary square root, preserving the retained energy magnitude. A downward liquid-liquid displacement also converts the gravitational potential omitted by submerged liquid gravity into a momentum-balanced transverse eddy; powders already receive buoyant gravity before using the same kinetic exchange. Moving particles are never rate-gated; a passive exchange between two liquids uses the slower participant's rate scaled by the square root of their normalized density contrast. Gases author drag, turbulence, and an optional flow rate. An omitted liquid flow rate is unrestricted, so the finite participant limits a pair and two unrestricted participants exchange every opportunity. Top-level fields pair by reader: `restitution` is contact bounce, read today by cell collisions and shared with rigid contacts by design; `entity_grip` and `entity_bounce` are how a surface feels underfoot. Plus density, hardness, and contact damage. Fuels author ignite, sealed fraction, burn rate, emit, palette, residue, burnout, and damage; hand flames author the burning side directly.
