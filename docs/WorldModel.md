...
`content/reactions.rs` holds every transmutation: pairwise `LAVA + WATER => STONE + STEAM @ 97.0` (material/tag operands, per-second rate) and decays `STEAM => WATER @ 0.1`. Combustion is **not** a reaction: a flammable material authors a `burn_variant` block and the macro synthesizes its ember twin (see [Simulation.md](Simulation.md)).
