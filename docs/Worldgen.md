...
Layout: a surface band at y≈0 (noise heightmap), infinite depth in progressive bands (biome/hazard/loot keyed on Y), infinite sky thinning above. Pipeline: base terrain (fBm + domain warping) → biome → material fill → features/structures → post-passes (ores, vegetation).
