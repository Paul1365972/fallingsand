...
fn star_layer(g: vec2<f32>, threshold: f32, time: f32) -> vec3<f32> {
    ...
    let d = length(f - pos);
    let core = 1.0 - smoothstep(0.0, 0.014 + 0.02 * mag, d);
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    let c = vec2<f32>((in.uv.x - 0.5) * params.aspect, params.horizon - in.uv.y);
    let th = params.sidereal * TAU;
    let ct = cos(th);
    let st = sin(th);
    let q = vec2<f32>(c.x * ct + c.y * st, -c.x * st + c.y * ct);
    var stars = vec3<f32>(0.0);
    stars = stars + star_layer(q * 12.0, 0.82, params.time) * 2.6;
    stars = stars + star_layer(q * 20.0 + vec2<f32>(31.0, 17.0), 0.88, params.time) * 1.7;
    stars = stars + star_layer(q * 32.0 + vec2<f32>(53.0, 61.0), 0.92, params.time) * 1.0;
