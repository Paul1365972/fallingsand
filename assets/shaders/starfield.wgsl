...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    let uv = vec2<f32>(in.uv.x * params.tiling, in.uv.y * params.tiling / params.aspect);
    let sample = textureSample(tex, tex_sampler, uv);

    let cell = floor(uv * 512.0);
    ...
    let alpha = clamp(sample.a * params.star_alpha * above * flicker, 0.0, 1.0);
